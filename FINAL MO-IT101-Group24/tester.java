...
public class tester {
    ...
    /*------------------------------------------------------------------------
        ...
        methods share the same input stream. Creating multiple Scanner objects
        on System.in can cause runtime conflicts and unpredictable input behavior
        because each instance competes to read from the same source.
    -------------------------------------------------------------------------*/
    ...
    /*------------------------------------------------------------------------
        ...
        updated in one place rather than hunting through every method.
    -------------------------------------------------------------------------*/
    ...
    public static void main(String[] args) {
        ...
        if (isEmployee) {
            ...
            if (option.equals("1")) {
                ...
                // "employeeNo" is the standardized variable name used throughout
                // the program to refer to an employee number, replacing the
                // previously inconsistent names: inputEmployeeNumber, empNum, empNo.
                String employeeNo = sc.nextLine();

                // These variables will hold the matched employee's data once found
                String  lastName  = "";
                String  firstName = "";
                String  birthday  = "";
                boolean found     = false;

                // Open the employee CSV and search for a row whose first column
                // matches the employee number entered by the user
                try (BufferedReader br = new BufferedReader(new FileReader(EMP_FILE))) {

                    // The first line is a header row (column labels), not actual data — skip it
                    br.readLine();
                    String line;
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        if (data[0].equals(employeeNo)) {
                            ...
                            found     = true;
                            break; // match found — no need to keep reading the file
                        }
                    }
                ...
                // Per process flow: display Employee #, Employee Name, and Birthday if found
                if (found) {
                    System.out.println("\n==================================== ");
            ...
            /*--------------------------------------------------------------------
                Option 2 — Exit
                The employee chose to exit. Close the scanner to release the
            ...
            ---------------------------------------------------------------------*/
                ...
                // Any input other than "1" or "2" is not a valid menu option
        ...
        -------------------------------------------------------------------------*/
            ...
            if (option.equals("1")) {
                ...
                /*----------------------------------------------------------------
                    ...
                    Per cutoff output:
                      First Cutoff  (1–15):  Total Hours Worked, Gross, Net
                ...
                if (subOption.equals("1")) {
                    ...
                    String employeeNo = sc.nextLine(); // standardized name: employeeNo
                    oneEmployee(employeeNo);

                /*----------------------------------------------------------------
                    Sub-option 2 — All Employees
                    Delegates to allEmployee(), which processes every employee
                    in the CSV using the same display format as Sub-option 1.
                -----------------------------------------------------------------*/
                ...
                -----------------------------------------------------------------*/
                    ...
                    // Any input other than "1", "2", or "3" is not valid
    ...
    ==========================================================================================*/
        ...
        /**
        * Computes the SSS (Social Security System) contribution of an employee
        * based on their combined monthly gross salary.
        *
        * Algorithm:
        * The SSS contribution table is stored in sss.csv. Each row in the file
    ...
    public static double computeSSS(double monthlyGross) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(SSS_FILE))) {
            ...
            br.readLine(); // first line is the column header row — skip it
            String line;
            ...
            while ((line = br.readLine()) != null) {
                ...
                String[] data        = line.split(",");
                double rangeFrom     = Double.parseDouble(data[0].trim()); // column 0: lower bound of bracket
                String rangeToText   = data[1].trim(); // column 1: upper bound, or "Over" for the last bracket
                double employeeShare = Double.parseDouble(data[3].trim()); // column 3: employee contribution for this bracket
                ...
                if (rangeToText.equalsIgnoreCase("Over")) {
                    ...
                    // Check if the monthly gross falls within this bracket's range (inclusive)
    ...
    ==========================================================================================*/
        ...
        /**
        * Computes the Pag-IBIG (Home Development Mutual Fund) contribution of an
        * employee based on their combined monthly gross salary.
        *
        * Algorithm:
        * The Pag-IBIG contribution table is stored in pagibig.csv. Each row defines
    ...
    public static double computePagibig(double monthlyGross) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(PAGIBIG_FILE))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String[] data    = line.split(",");
                double rangeFrom = Double.parseDouble(data[0].trim()); // column 0: lower bound of bracket
                double rangeTo   = Double.parseDouble(data[1].trim()); // column 1: upper bound of bracket
                double rate      = Double.parseDouble(data[2].trim()); // column 2: contribution rate (e.g., 0.01 = 1%)
                ...
                if (monthlyGross >= rangeFrom && monthlyGross <= rangeTo) {
                    ...
                    break; // bracket found — no need to read further rows
                }
            }
    ...
    ==========================================================================================*/
        ...
        /**
        * Computes the PhilHealth (Philippine Health Insurance) contribution of an
        * employee based on their combined monthly gross salary.
        *
        * Algorithm:
    ...
    public static double computePhilhealth(double monthlyGross) {
        ...
        // The variable name is kept fully descriptive to avoid the inconsistency
        // of the previous name "PHdeduction" which mixed abbreviation with camelCase.
    ...
    ==========================================================================================*/
        ...
        *
        *   Bracket 1: taxable ≤ 20,832              → Tax = 0.00 (exempt)
        ...
        *   on the remaining taxable salary. Order of operations matters here.
        *
        * @param monthlyGross      the combined gross salary of both cutoffs for the month
    ...
    public static double withholdingTax(double monthlyGross, double totalContribution) {
        double tax = 0.00;
        ...
        double taxableMonthlySalary = monthlyGross - totalContribution;
        ...
        if (taxableMonthlySalary <= 20832) {
            ...
            tax = (taxableMonthlySalary - 20833) * 0.20;
    ...
    ==========================================================================================*/
        ...
        *   Example: logs out 5:30 PM → treated as 5:00 PM.
        *
        * Rule 2 — Grace period:
        ...
        *   Example: logs in 8:05 AM → treated as 8:00 AM.
        *   Example: logs in 8:30 AM → actual time used (after grace period, no adjustment).
        *
    ...
    public static double computeHoursWorked(LocalTime logIn, LocalTime logOut) {
        ...
        // Convert minutes to hours (e.g., 450 minutes → 7.5 hours) and return
    ...
    /*========================================================================================
        Payroll Computation and Display (Method # 7) [rosella & ann]
    ==========================================================================================*/
        ...
        *
        * This method exists to eliminate the duplicate payroll logic that previously
        * appeared in both oneEmployee() and allEmployee(). Both methods now delegate
        * to this single shared method instead of each carrying their own copy of the
        * monthly loop. Any future change to the payroll logic only needs to be made here.
        *
        ...
        *
        * @param employeeNo        the employee number (consistent name used throughout)
        * @param lastName          the employee's last name
        * @param firstName         the employee's first name
        * @param birthday          the employee's birthday
        * @param rate              the employee's hourly rate from column 18 of details.csv
        * @param attendanceRecords all attendance records pre-loaded into memory
        * @param timeFormat        DateTimeFormatter for parsing H:mm time values from the CSV
        */
    ...
    public static void oneEmployee(String employeeNo) {
        ...
        // These will be populated once the correct employee row is found in the CSV
        String  lastName  = "";
        String  firstName = "";
        String  birthday  = "";
        boolean found     = false;
        ...
        // Search the employee file for a record matching the given employee number
        try (BufferedReader br = new BufferedReader(new FileReader(EMP_FILE))) {
            br.readLine(); // first line is the column header row — skip it
            String line;
            ...
            while ((line = br.readLine()) != null) {
                ...
                // A plain split(",") would break on names like "Dela Cruz, Juan".
                String[] data = line.split(",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");
                ...
                if (data[0].equals(employeeNo)) {
                    ...
                    found     = true;
                    break; // match found — stop reading the file
                }
            }
        ...
        // This way the attendance file is opened only once instead of once per month.
    ...
    public static void allEmployee() {
        ...
        // This list will hold one String[] per employee row read from details.csv
        List<String[]> employees = new ArrayList<>();

        // Step 1: Read all employee records from the employee file into memory
        try (BufferedReader br = new BufferedReader(new FileReader(EMP_FILE))) {
            br.readLine(); // first line is the column header row — skip it
            String line;
        ...
        // Passing this pre-loaded list to processPayroll() means the attendance file
        // is read only once for all employees, not once per employee per month.
        List<String[]> attendanceRecords = new ArrayList<>();
        try (BufferedReader br = new BufferedReader(new FileReader(ATT_FILE))) {
            br.readLine(); // first line is the column header row — skip it
            String line;
